...
def DeclRaToIndex(RA, decl, NSIDE=128): # find the index of (RA, Dec) in units of radians
...
dis_pointing = np.degrees(angular_dist(ra, dec, DeclRaToIndex(np.deg2rad(-116.6288746), np.deg2rad(-26.6969812))))
for i in range(len(fss)):
...
im2 = plot.healpix_contour(horizon, dlon=-np.deg2rad(116.62), linewidths=1, linestyles='-', colors='red')
##im3 = plot.healpix_contour(beam_4, dlon=-np.deg2rad(116.62), linewidths=1, colors='cyan')
im4 = plot.healpix_contour(beam_full, dlon=-np.deg2rad(116.62), linewidths=1, colors='gray')
...
ax.scatter(-(np.deg2rad(116.6288746)-ra[147827]), dec[147827], color='red', marker='+', s=100)
