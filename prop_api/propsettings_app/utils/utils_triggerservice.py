...
# Testing Django service - must be used in 'pretend' mode, as it's using a read-only database connection
BASEURL = os.getenv("MWA_BASEURL")
...
def web_api(
    ...
    print("DEBUG - calling web_api")

    print("DEBUG - urldict: ", urldict)
    ...
    try:
        if (username is not None) and (password is not None):
            ...
            print("DEBUG - sending request with auth")
            req = Request(
                url,
                postdata,
                {
            ...
            print("DEBUG - sending request without auth")
            req = Request(
                url,
                postdata,
...
def triggerbuffer(
    ...
    print("DEBUG - starting triggerbuffer()")
    ...
    print("BASEURL: ", type(BASEURL), ":", BASEURL)

    print(BASEURL + "triggerbuffer")

    result = web_api(
        url=BASEURL + "triggerbuffer", urldict=urldict, postdict=postdict, logger=logger
    )

    print("DEBUG - end triggerbuffer()")
    return result
