...
@log_event(log_location="end",message=f"Position uncertainty is large, not observing.", level="debug")
def check_large_position_error(telescope_settings: BaseTelescopeSettings, context: dict) -> dict:
    ...
    if ( telescope_settings.maximum_position_uncertainty
        and event.pos_error > telescope_settings.maximum_position_uncertainty) is False:
        return context
...
def check_atca_declination_limits(telescope_settings: BaseTelescopeSettings, context: dict) -> dict:
...
def check_fermi_likelihood(telescope_settings: BaseTelescopeSettings, context: dict) -> dict:
...
def check_swift_significance(telescope_settings: BaseTelescopeSettings, context: dict) -> dict:
...
def check_hess_significance(telescope_settings: BaseTelescopeSettings, context: dict) -> dict:
...
@log_event(message=f"Assuming it is a GRB if no likelihood data is available", level="debug")
...
def check_any_event_duration(telescope_settings: BaseTelescopeSettings, context: dict) -> dict:
...
def check_not_any_event_duration(telescope_settings: BaseTelescopeSettings, context: dict) -> dict:
...
def check_duration_with_limits(telescope_settings: BaseTelescopeSettings, context: dict) -> dict:
    if context["stop_processing"]:
        return context
...
def check_duration_with_limits_trigger(telescope_settings: BaseTelescopeSettings, context: dict) -> dict:
...
@log_event(message=f"Event duration within limits 1, waiting for a human's decision.", level="debug")
def check_duration_with_limits_pending1(telescope_settings: BaseTelescopeSettings, context: dict) -> dict:
...
@log_event(message=f"Event duration within limits 2, waiting for a human's decision.", level="debug") 
def check_duration_with_limits_pending2(telescope_settings: BaseTelescopeSettings, context: dict) -> dict:
...
@log_event(message=f"Event duration outside of all time ranges, not triggering.", level="debug")
def check_duration_with_limits_debug(telescope_settings: BaseTelescopeSettings, context: dict) -> dict:
    if context["stop_processing"]:
        return context
