...
from .utils_worth_observing import (
    check_worth_observing,
    make_trigger_decision,
    trigger_repointing,
)
...
@log_event(
    log_location="end",
    message=f"process_ignored_or_error_decision completed",
    level="info",
)
...
def process_trigger_decision(context):
    ...
    if prop_dec.ra and prop_dec.dec:
        ...
        print(
            "DEBUG - prop_dec.proposal.telescope_settings.repointing_limit:",
            prop_dec.proposal.telescope_settings.repointing_limit,
        )
        if event_sep > prop_dec.proposal.telescope_settings.repointing_limit:
            ...
            raise ValueError(
                "Not repointing: Event separation is within the repointing limit. Logic is not implemented yet."
            )
