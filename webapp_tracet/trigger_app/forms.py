        ...
        if self.cleaned_data["type"] == 1 or self.cleaned_data["type"] == 2:
            ...
            try:
                ...
                raise forms.ValidationError(
                    "Error sending test text message. Please ensure you have included your area code and verified your number on Twilio as explained in https://tracet.readthedocs.io/en/latest/new_user.html#verifying-your-phone-number-on-twilio"
                )
...
class ProjectSettingsForm(forms.ModelForm):
    def clean(self):
        # Check proposal ID
        if (
            len(self.cleaned_data["proposal_id"]) < 6
            or self.cleaned_data["proposal_id"] is None
        ):
            raise forms.ValidationError(
                "Please create a proposal ID with at least 6 character"
            )
        ...
        if "telescope" in self.cleaned_data.keys():
            ...
            if str(telescope).startswith("MWA"):
                ...
                if self.cleaned_data["mwa_freqspecs"]:
                    ...
                    raise forms.ValidationError(
                        {
                            "mwa_freqspecs": "No Frequency channel Specifications suppled."
                        }
                    )
                ...
                if (
                    not self.cleaned_data["atca_band_3mm"]
                    and not self.cleaned_data["atca_band_7mm"]
                    and not self.cleaned_data["atca_band_15mm"]
                    and not self.cleaned_data["atca_band_4cm"]
                    and not self.cleaned_data["atca_band_16cm"]
                ):
                    raise forms.ValidationError(
                        "Please choose at least 1 ATCA frequency band."
                    )
                ...
                if band1:
                    ...
                    atca_freq_bands(
                        83000,
                        105000,
                        self.cleaned_data["atca_band_3mm_freq1"],
                        "atca_band_3mm_freq1",
                    )
                    atca_freq_bands(
                        83000,
                        105000,
                        self.cleaned_data["atca_band_3mm_freq2"],
                        "atca_band_3mm_freq2",
                    )
                ...
                if band2:
                    atca_freq_bands(
                        30000,
                        50000,
                        self.cleaned_data["atca_band_7mm_freq1"],
                        "atca_band_7mm_freq1",
                    )
                    atca_freq_bands(
                        30000,
                        50000,
                        self.cleaned_data["atca_band_7mm_freq2"],
                        "atca_band_7mm_freq2",
                    )
                ...
                if band3:
                    atca_freq_bands(
                        16000,
                        25000,
                        self.cleaned_data["atca_band_15mm_freq1"],
                        "atca_band_15mm_freq1",
                    )
                    atca_freq_bands(
                        16000,
                        25000,
                        self.cleaned_data["atca_band_15mm_freq2"],
                ...
                if band4:
                    atca_freq_bands(
                        3900,
                        11000,
                        self.cleaned_data["atca_band_4cm_freq1"],
                        "atca_band_4cm_freq1",
                    )
                    atca_freq_bands(
                        3900,
                        11000,
                        self.cleaned_data["atca_band_4cm_freq2"],
                        "atca_band_4cm_freq2",
                    )
